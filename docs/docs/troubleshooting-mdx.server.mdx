...
*   [Problems writing MDX](#problems-writing-mdx)
    ...
    *   [`Could not parse import/exports with acorn: SyntaxError: Unexpected token`](#could-not-parse-importexports-with-acorn-syntaxerror-unexpected-token)
...
It occurs when the keywords `import` or `export` are found at the start of a
line but include characters reserved for Typescript.
An example is:

```mdx chrome=no
export const data: TableData[] = [{a:"b"}]
```

The reason for this error is that the parser is expecting a JavaScript import
or export statement.  Adding a type declaration introduces invalid characters.
